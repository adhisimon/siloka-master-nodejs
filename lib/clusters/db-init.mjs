...
const initClusterSchema = async (pool) => {
  try {
    ...
    if ((warningCount || 0) > 0) {
      const eventId = 'CE5F2EF6';
      logger.debug(`${module} ${eventId}: No need to create cluster_leader table`, { module, eventId });
    } else {
      const eventId = 'EE9E2FB8';
      logger.debug(`${module} ${eventId}: cluster_leader table created`, { module, eventId });
    }
  } catch (e) {
    const newE = new Error('Exception on initializing cluster schema');
    const eventId = '6A8F1405';

    logger.error(`${module} ${eventId}: ${newE.message}`, {
      module,
      eventId,
      eCode: e.code,
