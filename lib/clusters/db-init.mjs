...
const initClusterSchema = async (pool) => {
  try {
    ...
    if ((warningCount || 0) > 0) {
      ...
      logger.debug({ module, eventId }, `${module} ${eventId}: No need to create cluster_leader table`);
    } else {
      const eventId = 'EE9E2FB8';
      logger.debug({ module, eventId }, `${module} ${eventId}: cluster_leader table created`);
    }
  } catch (e) {
