...
const recordHeartbeat = (endpoint, metadata = {}) => {
  ...
  const existing = roster.get(endpoint);
  ...
  for (const node of roster.values()) {
    if (now - node.lastSeen <= ttlMs) {
      const nodeData = {
        ...
        activeSince: new Date(node.activeSince).toISOString()
      };
    ...
    for (const [endpoint, node] of roster.entries()) {
      if (now - node.lastSeen > ttlMs) {
        ...
        logger.debug({
          ...
          eventId: '9F82C101',
          endpoint
        }, `Purged expired ${node.component || 'unknown component'} node from roster: ${endpoint}`);
      }
    }
  }, intervalMs);
};
